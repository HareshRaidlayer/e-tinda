...
class HomeController extends Controller
{
    ...
    public function service(Request $request, $slug)
    {
        ...
        // $detailedService  = Service::where('slug', $slug)->where('status', 1)->first();
        $detailedService = Service::select(
            'services.*',
            DB::raw('users.name as sellername'),
            DB::raw('users.email as selleremail'),
            DB::raw('users.address as selleraddress'),
            DB::raw('users.city as sellercity'),
        ...
        if ($detailedService != null) {
            ...
            return view('frontend.service_details', compact('detailedService'));
        }
        abort(404);
    }
