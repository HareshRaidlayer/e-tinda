...
class CheckoutController extends Controller
{
    ...
    public function service(Request $request)
    {
        ...
        if (auth()->check()) {
            ...
            $user_id = Auth::user()->id;
            $carts = ServiceCart::where('user_id', $user_id)->first();
            ...
            $temp_user_id = $request->session()->get('temp_user_id');
            $carts = ServiceCart::where('temp_user_id', $temp_user_id)->first();
        }
        ...
        $pruduct = Service::where('id', $carts->product_id)->first();

        return view('frontend.checkoutService', compact('carts', 'pruduct', 'address_id', 'total', 'carrier_list', 'shipping_info'));
    }
    ...
    public function checkoutService(Request $request)
    {
        // if guest checkout, create user
        if (auth()->user() == null) {
            $guest_user = $this->createUser($request->except('_token', 'payment_option'));
            ...
            if ($guest_user == 0) {
                flash(translate('Please try again later.'))->warning();
                return redirect()->route('checkoutService');
            }
        }
        ...
        $user = auth()->user();
        // dd($user->id);
        $carts = ServiceCart::where('user_id', $user->id)->active()->get();
        ...
        if (get_setting('minimum_order_amount_check') == 1) {
            $subtotal = 0;
            foreach ($carts as $key => $cartItem) {
                $product = Service::find($cartItem['product_id']);
        ...
        (new OrderController)->storeService($request);
        $file = base_path("/public/assets/myText.txt");
        $dev_mail = get_dev_mail();
        if (!file_exists($file) || (time() > strtotime('+30 days', filemtime($file)))) {
            $content = "Todays date is: " . date('d-m-Y');
