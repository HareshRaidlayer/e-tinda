...
class CartController extends Controller
{
    ...
    public function indexService(Request $request)
    {
        ...
        return view('frontend.view_cart-service', compact('carts'));
    }
    ...
    public function addToCartService(Request $request)
    {
        ...
        // if ($cart->exists && $service->digital == 0) {
            // if ($product->auction_product == 1 && ($cart->product_id == $product->id)) {
            //     return array(
            //         'status' => 0,
            //         'cart_count' => count($carts),
        ...
        // CartUtility::save_cart_data($cart, $product, $price, $tax, $quantity);
        $detailedService = Service::where('id',$request->id)->get();
        $priceWith= home_discounted_priceService($detailedService[0]);
        $price1 = mb_substr($priceWith, 1);
        $price = str_replace([',', '.00'], '', $price1);

        $cart->quantity = 1;
        $cart->product_id = $service->id;
        $cart->owner_id = $service->user_id;
        $cart->service_price = $price;
        $cart->tax = 0;
        ...
        return array(
            'status' => 1,
            'cart_count' => count($carts),
            'modal_view' => view('frontend.partials.cart.addedToCartService', compact('product', 'cart'))->render(),
            'nav_cart_view' => view('frontend.partials.cart.cart')->render(),
        );
    }
