...
if (!function_exists('discount_in_percentage_service')) {
    function discount_in_percentage_service($product)
    {
        $base = home_base_price($product, false);
        $reduced = home_discounted_base_price($product, false);
        $discount = $base - $reduced;
...
if (!function_exists('home_price_service')) {
    function home_price_service($product, $formatted = true)
    {
...
if (!function_exists('home_discounted_priceService')) {
    function home_discounted_priceService($product, $formatted = true)
    {
        $lowest_price = $product['price'];
        $highest_price = $product['price'];
...
if (!function_exists('get_single_service')) {
    function get_single_service($product_id)
    {
        $product_query = Service::query()->with('thumbnail');
        return $product_query->find($product_id);
    }
}
...
if (!function_exists('get_level_zero_categories')) {
    function get_level_zero_categories()
    {
        ...
        return $categories_query->where('parent_id', 12)->orderBy('order_level', 'desc')->get();
    }
}
// Get categories by level zero
if (!function_exists('get_level_zero_service')) {
    function get_level_zero_service()
    {
        $service_query = Service::where('status', 1)->orderBy('id', 'desc')->get();
        return $service_query;
    }
}
if (!function_exists('get_level_zero_farmer')) {
    function get_level_zero_farmer()
    {
        $farmerProduct = Product::where('published', 1)->where('category_id', 11)->get();
