                ...
                <ul class="list-unstyled">
                    @foreach (get_level_zero_categories()->take(10) as $key => $category)
                        ...
                        <li class="p-2 sub-mrnu-custom"><a
                                href="{{ route('products.category', $category->slug) }}">{{ $category_name }}</a></li>
                    @endforeach
                </ul>
            </div>
        </li>
...
<div class="aiz-category-menu bg-white rounded-0 border-top shopee" id="category-sidebar" style="width:270px;">
    <ul class="list-unstyled categories no-scrollbar mb-0 text-left">
        <?php $Ebakasyon = get_single_category(15);
        ?>
        <li class="category-nav-element border border-top-0 position-relative">
            <a href="{{ route('propertyList') }}" class="text-truncate text-dark px-4 fs-14 d-block hov-column-gap-1">
                <img class="cat-image lazyload mr-2 opacity-60" src="{{ static_asset('assets/img/placeholder.jpg') }}"
            ...
            <div class="sub-cat-menu c-scrollbar-light border p-2 shadow-none bg-white">
                <ul class="list-unstyled">
                    @foreach (get_level_zero_ebakasyon() as $key => $ebakasyonProduct)
                        @php
                            $service_name = $ebakasyonProduct['name'];
                        @endphp
                        <li class="p-2 sub-mrnu-custom"><a
                                href="{{ url('/property/list?category=' . $ebakasyonProduct->id) }}">{{ translate($service_name) }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
