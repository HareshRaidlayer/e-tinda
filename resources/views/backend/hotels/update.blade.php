...
@section('panel_content')
    <div class="aiz-titlebar mt-2 mb-4">
        <div class="row align-items-center">
            <div class="col-md-6">
                <h1 class="h3">{{ translate('Update Hotel and Rooms') }}</h1>
            </div>
        </div>
    </div>
    <!-- Error Messages -->
    ...
    @endif
    <form action="{{ route('seller.hotels.update', ['id' => $hotel->id]) }}" method="POST" enctype="multipart/form-data" id="hotel_room_form">
        @csrf
                        ...
                        <div class="card-body">
                            <div class="form-group row">
                                ...
                                <div class="col-lg-9 mb-2">
                                    <input type="text" class="form-control" name="name" value="{{ $hotel->name }}" placeholder="{{ translate('Hotel Name') }}">
                                    @error('name')
                                ...
                                <div class="col-lg-9 mb-2">
                                    <select class="form-control aiz-selectpicker rounded-0" data-live-search="true" data-placeholder="{{ translate('Select your country') }}" name="category_id" required>
                                        ...
                                        @foreach ($categories as $key => $category)
                                            <option value="{{ $category->id }}" <?php echo ($category->id == $hotel->category_id ? 'selected' : '') ;?>>{{ $category->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-9 mb-2">
                                    <input type="text" class="form-control" name="address" value="{{ $hotel->address }}" placeholder="{{ translate('Address') }}">
                                    @error('address')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <div class="input-group" data-toggle="aizuploader" data-type="image">
                                        ...
                                        <input type="hidden" name="image" value="{{ $hotel->image }}" class="selected-files">
                                    </div>
                                ...
                                <div class="room-row">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-lg-4 mb-2">
                                            <input type="text" class="form-control" name="rooms[{{ $index }}][room_number]" value="{{ $room->room_number }}" placeholder="{{ translate('Room Number') }}">
                                        </div>
                                        <label class="col-lg-2 col-form-label">{{ translate('Capacity') }}</label>
                                        <div class="col-lg-4 mb-2">
                                            {{-- <input type="text" class="form-control" name="rooms[{{ $index }}][capacity]" value="{{ $room->capacity }}" placeholder="{{ translate('Capacity') }}"> --}}
                                            <select class="form-control" name="rooms[{{ $index }}][capacity]">
                                        ...
                                        <div class="col-lg-4 mb-2">
                                            <input type="number" class="form-control" name="rooms[{{ $index }}][price]" value="{{ $room->price }}" placeholder="{{ translate('Price') }}">
                                        </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group" data-toggle="aizuploader" data-type="image">
                                                ...
                                                <div class="form-control file-amount">{{ translate('Choose File') }}</div>
                                                <input type="hidden" name="rooms[{{ $index }}][images]" value="{{ $room->images }}" class="selected-files">
                                            </div>
                        ...
                        <div class="card-header">
                            <div class="col-12">
                                <div class="mar-all text-right mb-2">
                                    <button type="submit" name="button" value="publish" class="btn btn-primary">{{ translate('Submit') }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('script')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            let roomCount = {{ count($hotel->rooms) }};
